...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Portrait — Option 4: full-height photo on the right edge, blended into the background */}
      <motion.div
        aria-hidden
        initial={{ opacity: 0, y: 24, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 1.2, delay: 0.2, ease: 'easeOut' }}
        className="pointer-events-none absolute inset-y-0 right-0 z-0 hidden sm:block sm:w-[70%] md:w-[58%] lg:w-[52%]"
      >
        {/* Photo */}
        ...
        <img
          ...
          alt=""
          className="absolute inset-0 w-full h-full object-contain object-right-bottom opacity-40 md:opacity-80"
          style={{ filter: 'saturate(0.95) contrast(1.05) brightness(0.95)' }}
        />

        {/* Cool tonal cast — ties the photo into the violet/cyan theme */}
        <div
          className="absolute inset-0 mix-blend-color opacity-70"
          style={{
            background:
              'linear-gradient(135deg, rgba(99,102,241,0.6), rgba(34,211,238,0.35) 50%, rgba(139,92,246,0.6))',
          }}
        />

        {/* Left-edge fade — keeps centered text fully readable */}
        <div
          className="absolute inset-y-0 left-0 w-2/3 md:w-1/2"
          style={{
            background:
              'linear-gradient(90deg, rgb(15,23,42) 0%, rgba(15,23,42,0.92) 25%, rgba(15,23,42,0.55) 55%, transparent 100%)',
          }}
        />

        {/* Bottom fade — blends legs/feet into the page */}
        <div
          className="absolute inset-x-0 bottom-0 h-1/3"
          style={{
            background:
              'linear-gradient(180deg, transparent 0%, rgba(11,17,36,0.6) 60%, rgb(11,17,36) 100%)',
          }}
        />

        {/* Top fade — softens the hairline so it doesn't cut hard against the navbar area */}
        <div
          className="absolute inset-x-0 top-0 h-24"
          style={{
            background:
              'linear-gradient(180deg, rgba(11,17,36,0.7) 0%, transparent 100%)',
          }}
        />
        ...
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 80% 70% at 80% 45%, transparent 35%, rgba(11,17,36,0.45) 80%, rgba(7,11,24,0.85) 100%)',
          }}
        />
      </motion.div>
        ...
        >
          ...
          >
            ...
            <div className="relative flex flex-col items-center gap-0.5 text-white">
              ...
              <span className="text-[9px] font-medium tracking-widest uppercase text-white/70">
                {label}
              </span>
            </div>
          </motion.div>
      ...
      <div className="section relative z-10 flex flex-col items-center text-center">
        ...
        <motion.h1
          ...
          className="mt-6 font-display font-semibold tracking-tight text-white leading-[1.15] sm:leading-[1.02] text-[2rem] sm:text-6xl lg:text-7xl text-balance"
        >
        ...
        <motion.p
          ...
          className="mt-7 max-w-2xl text-white/70 text-base sm:text-lg leading-relaxed text-pretty"
        >
